...
//     // RAII Resource Acquisition is Initialization (interview question from slack)
//     {
...
{ // thread runner
    ...
    // Dont know if mutex lock is needed
    //pthread_mutex_lock(&mutex_);
    std::cout << ">>>>>>>>>>>>>>> Customer thread p#" << c->get_id() << " shutting down... <<<<<<<<<<<<<<<<<\n\n";
    b->show();
    //pthread_mutex_unlock(&mutex_);
...
{ // processes all files in command line
    while (--argc > 0)
