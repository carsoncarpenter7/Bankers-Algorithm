...
public:
    ...
    //  Customer() = default;
    // Constructor with random request (fix where called)
    ...
    void alloc_req(const ext_vector<int>& req)
    {
        alloc += req;
        need -= req;
    }
    void dealloc_req(const ext_vector<int>& req)
    {
        alloc -= req;
        need += req;
    }
